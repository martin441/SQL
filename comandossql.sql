...
SELECT * FROM actors
  WHERE last_name LIKE '%stack%'; /*podria usar 'stack%' o '%stack', el % indica que antes o despues de el, contenga ese string */
...
SELECT first_name  || ' ' ||  last_name as full_name, COUNT(*) as occurrences FROM actors
GROUP BY  full_name
ORDER BY occurrences DESC
LIMIT 10 OFFSET 0;
...
SELECT movies_genres.genre, COUNT(*) AS No_of_movies 
FROM movies_genres 
...
SELECT (directors.first_name || ' ' || directors.last_name) AS director_name, movies.name AS movie_name, movies.year 
FROM movies
